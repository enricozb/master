...
function refresh_tabs(on_complete) {
    chrome.runtime.sendMessage({request: 'tabsearch:request:get_tabs'}, function(response) {
        on_complete(response.concat(get_command_tabs()))
    })
}
...
function filter(tab, str, regex, num_filtered) {
    str = str.replace(/\s/g, '')
    ...
    if(match_result != null) {
        ...
        if(str.length > 0) {
            var priority = tab.title.toLowerCase().indexOf(str.toLowerCase())
            if(priority > -1) {
